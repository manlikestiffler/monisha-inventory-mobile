...
export const useInventoryStore = create((set, get) => ({
  ...
  setupRealtimeListeners: () => {
    ...
    const updateProductsState = async () => {
      try {
        // Extract creator info directly from product data - prefer full name over email
        const uniformsWithVariants = uniformsData.map(uniform => {
          ...
          return { 
            ...uniform, 
            variants,
            creatorName: uniform.createdByName || uniform.createdBy || 'N/A',
            creatorRole: uniform.createdByRole || 'staff' // Default role
          };
        });
        
        // Add creator info to materials - prefer full name over email
        const materialsWithCreatorInfo = materialsData.map(material => {
          return {
            ...material,
            creatorName: material.createdByName || material.createdBy || 'N/A',
            creatorRole: material.createdByRole || 'staff' // Default role
          };
        });
  ...
  fetchUniforms: async () => {
    // Alias for fetchProducts to maintain compatibility
          ...
          const updatedSizes = item.sizes.map(s => {
            if (s.size === size) {
              return {
                ...s,
                reorderLevel: reorderLevel,
                reorderQuantity: reorderQuantity || reorderLevel * 2
              };
            }
            return s;
          });
      ...
      for (const doc of batchSnapshot.docs) {
        ...
        batch.items?.forEach(item => {
          item.sizes?.forEach(sizeData => {
            const reorderLevel = sizeData.reorderLevel || 10;
            const quantity = sizeData.quantity || 0;
